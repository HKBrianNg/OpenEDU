// client/src/games/shooter/ShooterScene.ts
import Phaser from 'phaser';
import { generateTextures, levelConfig, LEVEL_FORMATIONS, ENEMY_TYPES } from './shooterConfig';
...
export class ShooterScene extends Phaser.Scene {
  ...
  private shieldPickups!: Phaser.Physics.Arcade.Group;  // 防护罩道具组
  private score = 0;
  private level = 1;
  private lives = 3;
  ...
  private readonly AUTO_FIRE_INTERVAL = 680;
  private explosionEmitter!: Phaser.GameObjects.Particles.ParticleEmitter;
  private flashOverlay!: Phaser.GameObjects.Rectangle;
  private isRespawning = false;
  private readonly RESPAWN_DELAY = 1260;
  private pickupEmitter!: Phaser.GameObjects.Particles.ParticleEmitter;
  private isLevelTransition = false;

  // 防护罩相关
  private shieldSprite!: Phaser.GameObjects.Sprite;  // 显示的护盾
  private shieldActive = false;
  private shieldRemainingTime = 0;  // 剩余毫秒
  private shieldDuration = 30000;   // 30秒
  ...
  create() {
    ...
    // 文本
    this.scoreText = this.add.text(10, 103, `${t('shooter.score')}: ${this.score}`, { fontSize: '18px', color: '#fff' });
    this.livesText = this.add.text(10, 129, `${t('shooter.lives')}: ${this.lives}`, { fontSize: '18px', color: '#f44' });
    this.levelText = this.add.text(10, 155, `${t('shooter.level')}: ${this.level}`, { fontSize: '18px', color: '#0ff' });
    ...
    this.physics.add.overlap(
      this.player, this.eggs,
      this.collectEgg as Phaser.Types.Physics.Arcade.ArcadePhysicsCallback,
      undefined, this
    );
    // 防护罩拾取重叠
    this.physics.add.overlap(
      this.player, this.shieldPickups,
  ...
  update(time: number) {
    if ((window as any).__paused) return;
    ...
    if (!this.sys.game.device.input.touch) {
      ...
      if (time > this.lastAutoFire + this.AUTO_FIRE_INTERVAL) {
        this.fireBullet();
    ...
    if (this.shieldActive) {
      ...
      // 倒计时
      const delta = this.game.loop.delta; // 上一帧经过的时间（ms）
      this.shieldRemainingTime -= delta;
    ...
    for (const e of enemyList) {
      if (e.active && e.y > camHeight + 106) {
        ...
        e.setVelocityY(type.speed * 0.353);
  ...
  private spawnFormation() {
    ...
    formation.rows.forEach((row, rowIndex) => {
      ...
      for (let i = 0; i < row.count; i++) {
        ...
        enemy.setVelocityY(type.speed * 0.351);
        this.tweens.add({
          targets: enemy,
  ...
  private levelClear() {
    ...
    if (this.level >= 10) {
      const cam = this.cameras.main;
      const txt = this.add.text(cam.centerX, cam.centerY, `${t('shooter.youWin')}\n${t('shooter.pressSpacePlayAgain')}`, {
    ...
    const infoText = this.add.text(cam.centerX, cam.centerY + 30, t('shooter.getReady'), {
      fontSize: '27px', color: '#ffffff',
    }).setOrigin(0.5).setDepth(516);

    this.time.delayedCall(2490, () => {
      this.level++;
      this.levelText.setText(`${t('shooter.level')}: ${this.level}`);
      overlay.destroy();
      levelUpText.destroy();
      infoText.destroy();
      this.physics.resume();
  ...
  private spawnEgg() {
    ...
    egg.setVelocityY(48 + this.level * 5);
  ...
  private playerHit(
    ...
    otherObj: Phaser.Types.Physics.Arcade.GameObjectWithBody
  ) {
    // 如果防护罩激活，则不扣血，只消耗防护罩
    if (this.shieldActive) {
      // 销毁撞击物（敌人或子弹）
      (otherObj as Phaser.Physics.Arcade.Sprite).destroy();
      // 取消防护罩（也可以选择不取消，只免疫一次；这里设计为免疫一次后护盾消失）
      this.deactivateShield();
      // 播放一个小特效表示格挡
    ...
    this.lives--;
    this.livesText.setText(`${t('shooter.lives')}: ${this.lives}`);
    ...
    if (this.lives <= 0) {
      ...
      this.time.delayedCall(this.RESPAWN_DELAY, () => {
  ...
  private gameOver() {
    const cam = this.cameras.main;
    this.physics.pause();
    const txt = this.add.text(cam.centerX, cam.centerY, `${t('shooter.gameOver')}\n${t('shooter.pressSpaceRestart')}`, {
